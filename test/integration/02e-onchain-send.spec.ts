...
import { PayOnChainClient } from "src/OnChain" // TODO
import { getTitle } from "src/notifications/payment"
import { Transaction } from "src/schema"
import { bitcoindDefaultClient, bitcoindHotWalletClient, btc2sat, sleep } from "src/utils"
import {
  checkIsBalanced,
  getUserWallet,
  lndonchain,
...
beforeEach(async () => {
  initBlockCount = await bitcoindHotWalletClient.getBlockCount()
...
afterAll(async () => {
  await bitcoindHotWalletClient.generateToAddress(3, RANDOM_ADDRESS)
  await sleep(2000)
...
it("SendsOnchainPaymentSuccessfully", async () => {
  ...
  ////////////////
  const complete_results = await MainBook.ledger({
    account: userWallet0.accountPath,
    pending: true,
  })

  const pendingTxn = complete_results.results[0]
  ////////////////
...

  {
    await Promise.all([
      ...
      bitcoindHotWalletClient.generateToAddress(6, RANDOM_ADDRESS),
    ])
  }
  ...
  // sendNotification.mock.calls is returning []
  ...
  // TODO! how to do the updates to pending false for bitcoind onchain?
  // expect(pending).toBe(false)

  const txnFee = await PayOnChainClient.clientPayInstance().getTxnFee(pendingTxn.hash)
  expect(fee).toBe(txnFee + userWallet0.user.withdrawFee)
